# +-----------------------------------------------------------+
# | Allow IAM Group 'developers' in main account to           |
# | sts:AssumeRole IAM Role 'developers' in target account    |
# +-----------------------------------------------------------+
...
resource "aws_iam_policy" "assume_env_developer_role" {
  ...
  path        = "/"
  description = "Allows assuming the developer role inte ${element(keys(var.aws_account_ids), count.index)} environment"
